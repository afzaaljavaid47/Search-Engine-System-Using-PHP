...
<li class="nav-item"><a class="nav-link" href="index1.php">Add New Room</a></li>
      <li class="nav-item"><a class="nav-link active" href="booked.php">Booked Room List</a></li>
    ...
    <table class="table table-striped table-responsive-md table-bordered">
      <thead class="bg-light">
        <tr>
          ...
          <th class="text-center" style="font-size: 20px;padding-top: 8px;">View Info</th>
        </tr>
      </thead>
  <style type="text/css">
    #head
...
include("dbcon.php");
$sql="SELECT * FROM `room` WHERE `status`='book'";
$run=mysqli_query($con,$sql);
while($data=mysqli_fetch_assoc($run))
{
  ?>
  <tbody style="background-color: rgba(0,0,0,0.2);color:white;font-size: 19px;font-weight: bold;letter-spacing: 2px;">
  <tr>
    ...
    <td class="text-center"><a href="gotobook.php?sid=<?php echo $data['room_id'];?>" class="btn btn-primary" style="font-size: 20px;">View Info</a></td>
  </tr>
  <?php
}
